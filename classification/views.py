...
def get_word_language(request):
    """
    Determine the language of the word - now defaults to Korean since we only support Korean → English
    """
    # Default to Korean since we only support Korean → English translation
    lang = 'ko'
    
    # Also check POST data for language context (if explicitly provided)
...
def update_learning_metrics(user, words_learned=0, words_reviewed=0, time_spent=0):
    """Update learning metrics for the user"""
    today = date.today()
    
    # Get or create metrics for today
    metrics, created = LearningMetrics.objects.get_or_create(
        user=user,
        date=today,
        defaults={
            'words_learned': 0,
    ...
    # Update metrics
    metrics.words_learned += words_learned
    metrics.words_reviewed += words_reviewed
    metrics.time_spent_minutes += time_spent
    metrics.save()
...
def get_user_vocabulary_dict(user, language='ko'):
    """
    Get user's vocabulary as a dictionary for quick lookups
    """
    vocabulary = {}
    
    for word in UserVocabulary.objects.filter(user=user, language=language):
